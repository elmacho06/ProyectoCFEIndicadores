...
if (isset($_GET['month'])) {
    ...
    if ($month === 'enero') {
        ...
        $sql = "SELECT indice, meta1 AS meta, tolerancia1 AS tolerancia, reales1 AS reales FROM chenalho LIMIT 12";
    } elseif ($month === 'marzo') {
        $sql = "SELECT indice, meta2 AS meta, tolerancia2 AS tolerancia, reales2 AS reales FROM acala LIMIT 12";
    } elseif ($month === 'abril') {
        $sql = "SELECT indice, meta3 AS meta, tolerancia3 AS tolerancia, reales3 AS reales FROM acala LIMIT 12";
    } elseif ($month === 'mayo') {
        $sql = "SELECT indice, meta4 AS meta, tolerancia4 AS tolerancia, reales4 AS reales FROM acala LIMIT 12";
    } elseif ($month === 'junio') {
        $sql = "SELECT indice, meta5 AS meta, tolerancia5 AS tolerancia, reales5 AS reales FROM acala LIMIT 12";
    } elseif ($month === 'julio') {
        $sql = "SELECT indice, meta6 AS meta, tolerancia6 AS tolerancia, reales6 AS reales FROM acala LIMIT 12";
    } elseif ($month === 'agosto') {
        $sql = "SELECT indice, meta7 AS meta, tolerancia7 AS tolerancia, reales7 AS reales FROM acala LIMIT 12";
    } elseif ($month === 'septiembre') {
        $sql = "SELECT indice, meta8 AS meta, tolerancia8 AS tolerancia, reales8 AS reales FROM acala LIMIT 12";
    } elseif ($month === 'octubre') {
        $sql = "SELECT indice, meta9 AS meta, tolerancia9 AS tolerancia, reales9 AS reales FROM acala LIMIT 12";
    } elseif ($month === 'noviembre') {
        $sql = "SELECT indice, meta10 AS meta, tolerancia10 AS tolerancia, reales10 AS reales FROM acala LIMIT 12";
    } 
    elseif ($month === 'diciembre') {
        $sql = "SELECT indice, meta11 AS meta, tolerancia11 AS tolerancia, reales11 AS reales FROM acala LIMIT 12";
    } 
...
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    ...
    padding: 10px 20px; /* Espaciado alrededor del contenido del botón */
    font-size: 16px; /* Tamaño de la fuente */
    font-weight: bold; /* Negrita */
    border-radius: 5px; /* Bordes redondeados */
    cursor: pointer; /* Cambia el cursor cuando pasas sobre el botón */
    display: inline-flex; /* Hace que el contenido se alinee correctamente */
...
.chart-button:hover {
    ...
    border-color: green; /* Mantener borde verde al pasar el mouse */
}
...
<body>
    <div class="navbar">
        ...
        <select id="pageSelect">
            ...
            <option value="../vistaagencia/indexChenalho.php">Página Chenalho</option>
            <option value="../VMensualChenalho/consulta_mensual_chenalho.php?month=enero">Enero</option>
            <option value="../VMensualChenalho/consulta_mensual_chenalho.php?month=febrero">Febrero</option>
            <option value="../VMensualChenalho/consulta_mensual_chenalho.php?month=marzo">Marzo</option>
            <option value="../VMensualChenalho/consulta_mensual_chenalho.php?month=abril">Abril</option>
            <option value="../VMensualChenalho/consulta_mensual_chenalho.php?month=mayo">Mayo</option>
            <option value="../VMensualChenalho/consulta_mensual_chenalho.php?month=junio">Junio</option>
            <option value="../VMensualChenalho/consulta_mensual_chenaho.php?month=julio">Julio</option>
            <option value="../VMensualChenalho/consulta_mensual_chenalho.php?month=agosto">Agosto</option>
            <option value="../VMensualChenalho/consulta_mensual_chenalho.php?month=septiembre">Septiembre</option>
            <option value="../VMensualChenalho/consulta_mensual_chenalho.php?month=octubre">Octubre</option>
            <option value="../VMensualChenalho/consulta_mensual_chenalho.php?month=noviembre">Noviembre</option>
            <option value="../VMensualChenalho/consulta_mensual_chenalho.php?month=diciembre">Diciembre</option>
        </select>
        ...
        <h1>"Transparencia y Eficiencia: Seguimiento de Indicadores en Chenalho"</h1>
        <button><a href="../vistaagencia/indexChenalho.php">Subir Avances</a></button> 
  ...
  max-width: 1600px; /* Controla el ancho máximo del contenido */
    ...
    <script>
        ...
        chartButton.addEventListener('click', function() {
            loadPage('../graficos_agencias_mensuales/consultar_datos_chenalho.php'); // Cargar contenido de enero.php
        });
